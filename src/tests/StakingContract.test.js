...
describe("StakingContract", function () {
    ...
    beforeEach(async function () {
        ...
        await testToken.connect(addr1).approve(stakingContract.address, ethers.utils.parseUnits("100", 18));
    });
    ...
    it("Should prevent unstaking before the lock period", async function () {
        ...
        await stakingContract.connect(addr1).stake(stakeAmount, lockPeriod);
        await expect(stakingContract.connect(addr1).unstake(0)).to.be.revertedWith("Stake is still locked");
    });
